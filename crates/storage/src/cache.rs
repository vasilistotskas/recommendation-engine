...
impl Default for RedisCacheConfig {
    fn default() -> Self {
        Self {
            ...
            // Increased pool size for very high concurrency (1000+ concurrent requests)
            pool_size: 50,
            // Longer timeout for high load scenarios
            connection_timeout: Duration::from_secs(3),
            // More retries to handle transient failures under load
...
impl RedisCache {
    ...
    pub async fn delete_pattern(&self, pattern: &str) -> Result<usize> {
        ...
        let deleted: i32 = conn.del(&keys).await.map_err(|e| {
            error!(
                "Failed to delete keys matching pattern '{}': {}",
                pattern, e
            );
            self.metrics.record_error();
            CacheError::ConnectionError(e)
...
pub mod cache_keys {
    ...
    // Increased TTLs for better cache hit rates under high load
    pub const RECOMMENDATION_TTL: Duration = Duration::from_secs(600); // 10 minutes (was 5)
    pub const TRENDING_TTL: Duration = Duration::from_secs(7200); // 2 hours (was 1)
    pub const USER_PREFERENCE_TTL: Duration = Duration::from_secs(900); // 15 minutes (was 10)
    pub const ENTITY_FEATURE_TTL: Duration = Duration::from_secs(7200); // 2 hours (was 1)
...
impl RedisCache {
    ...
    pub async fn cache_recommendations(
        ...
        self.set(&key, recommendations, cache_keys::RECOMMENDATION_TTL)
            .await
    }
    ...
    pub async fn cache_user_preference_vector(
        ...
        self.set(&key, vector, cache_keys::USER_PREFERENCE_TTL)
            .await
    }
    ...
    pub async fn invalidate_entity_caches(&self, entity_id: &str) -> Result<usize> {
        ...
        debug!(
            "Invalidated {} cache entries for entity: {}",
            total_deleted, entity_id
        );
        Ok(total_deleted)
    }
...
mod tests {
    ...
    fn test_ttl_constants() {
        assert_eq!(cache_keys::RECOMMENDATION_TTL, Duration::from_secs(600));
        assert_eq!(cache_keys::TRENDING_TTL, Duration::from_secs(7200));
        assert_eq!(cache_keys::USER_PREFERENCE_TTL, Duration::from_secs(900));
        assert_eq!(cache_keys::ENTITY_FEATURE_TTL, Duration::from_secs(7200));
    }
