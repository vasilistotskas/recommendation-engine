...
impl TestSystem {
    ...
    async fn cleanup_tenant(&self, tenant_id: &str) -> Result<()> {
        ...
        for entity in entities {
            let _ = self
                .vector_store
                .delete_entity(&ctx, &entity.entity_id, &entity.entity_type)
                .await;
        }
        ...
        let _ = self
            .redis_cache
            .delete_pattern(&format!("*{}*", tenant_id))
            .await;
...
async fn test_complete_workflow_from_entity_creation_to_recommendations() -> Result<()> {
    ...
    product_attributes_1.insert(
        "name".to_string(),
        AttributeValue::String("Wireless Headphones".to_string()),
    );
    product_attributes_1.insert(
        "category".to_string(),
    ...
    product_attributes_1.insert(
        "brand".to_string(),
        AttributeValue::String("TechBrand".to_string()),
    );
    ...
    product_attributes_2.insert(
        "name".to_string(),
        AttributeValue::String("Bluetooth Speaker".to_string()),
    );
    product_attributes_2.insert(
        "category".to_string(),
        AttributeValue::String("electronics".to_string()),
    ...
    product_attributes_3.insert(
        "name".to_string(),
        AttributeValue::String("Running Shoes".to_string()),
    );
    product_attributes_3.insert(
        "category".to_string(),
    ...
    product_attributes_3.insert(
        "brand".to_string(),
        AttributeValue::String("SportBrand".to_string()),
    );
    ...
    use recommendation_models::{Algorithm, RecommendationRequest};
    ...
    let collab_recommendations = system
        .recommendation_service
        .get_recommendations(&ctx, collab_request)
        .await?;

    assert_eq!(collab_recommendations.algorithm, "collaborative");
    // Should have recommendations (or cold start fallback)
    assert!(
        !collab_recommendations.recommendations.is_empty() || collab_recommendations.cold_start
    );
    ...
    let content_recommendations = system
        .recommendation_service
        .get_recommendations(&ctx, content_request)
        .await?;
    ...
    if !content_recommendations.recommendations.is_empty() {
        let recommended_ids: Vec<String> = content_recommendations
            .recommendations
            .iter()
        ...
        {
            let product_2_score = content_recommendations
                .recommendations
                .iter()
                .find(|r| r.entity_id == "product_2")
                .map(|r| r.score)
                .unwrap_or(0.0);
            let product_3_score = content_recommendations
                .recommendations
                .iter()
            ...
            assert!(
                product_2_score >= 0.0 && product_3_score >= 0.0,
    ...
    let hybrid_recommendations = system
        .recommendation_service
        .get_recommendations(&ctx, hybrid_request)
        .await?;

    assert_eq!(hybrid_recommendations.algorithm, "hybrid");
    assert!(
        !hybrid_recommendations.recommendations.is_empty() || hybrid_recommendations.cold_start
    );
...
async fn test_multi_tenancy_isolation() -> Result<()> {
    ...
    attributes_a.insert(
        "name".to_string(),
        AttributeValue::String("Product A".to_string()),
    );
    attributes_a.insert(
        "category".to_string(),
        AttributeValue::String("category_a".to_string()),
    );
    ...
    attributes_b.insert(
        "name".to_string(),
        AttributeValue::String("Product B".to_string()),
    );
    attributes_b.insert(
        "category".to_string(),
        AttributeValue::String("category_b".to_string()),
    );
    ...
    let result = system
        .entity_service
        .get_entity(&ctx_a, "product_b1".to_string(), "product".to_string())
        .await?;
    assert!(
        result.is_none(),
        "Tenant A should not see tenant B's entities"
    );

    // Verify tenant B cannot see tenant A's entities
    let result = system
        .entity_service
        .get_entity(&ctx_b, "product_a1".to_string(), "product".to_string())
        .await?;
    assert!(
        result.is_none(),
    ...
    let result = system
        .entity_service
        .get_entity(&ctx_a, "product_a1".to_string(), "product".to_string())
        .await?;
    ...
    let result = system
        .entity_service
        .get_entity(&ctx_b, "product_b1".to_string(), "product".to_string())
        .await?;
...
async fn test_all_algorithms() -> Result<()> {
    ...
    for i in 1..=10 {
        ...
        attributes.insert(
            "name".to_string(),
            AttributeValue::String(format!("Product {}", i)),
        );
        attributes.insert(
            "category".to_string(),
            AttributeValue::String(if i <= 5 {
        ...
        attributes.insert(
            "price".to_string(),
            AttributeValue::Number(50.0 + (i as f64 * 10.0)),
        );
        attributes.insert(
            "rating".to_string(),
            AttributeValue::Number(3.0 + (i as f64 * 0.2)),
        );
    ...
    println!(
        "Collaborative recommendations: {} items, cold_start: {}",
        collab_result.recommendations.len(),
        collab_result.cold_start
    );
    ...
    println!(
        "Content-based recommendations: {} items",
        content_result.recommendations.len()
    );
    ...
    if !content_result.recommendations.is_empty() {
        ...
        let electronics_count = content_result
            .recommendations
            .iter()
            .filter(|r| {
                let id_num: usize = r
    ...
    println!(
        "Hybrid recommendations: {} items, cold_start: {}",
        hybrid_result.recommendations.len(),
        hybrid_result.cold_start
    );

    // Verify all algorithms return results
    assert!(
        !collab_result.recommendations.is_empty() || collab_result.cold_start,
        "Collaborative should return recommendations or indicate cold start"
    );
    assert!(
        !content_result.recommendations.is_empty(),
        "Content-based should return recommendations"
    );
    assert!(
        !hybrid_result.recommendations.is_empty() || hybrid_result.cold_start,
    ...
    for rec in &collab_result.recommendations {
        assert!(
            rec.score >= 0.0 && rec.score <= 1.0,
            "Collaborative score should be in [0, 1]: {}",
            rec.score
        );
    }
    ...
    for rec in &content_result.recommendations {
        assert!(
            rec.score >= 0.0 && rec.score <= 1.0,
            "Content-based score should be in [0, 1]: {}",
            rec.score
        );
    }
