...
impl TenantConfigManager {
    ...
    pub fn load_from_file(&self, path: &str) -> Result<()> {
        ...
        let tenant_configs: HashMap<String, TenantConfig> = if path.ends_with(".json") {
            serde_json::from_str(&content).map_err(ConfigError::SerializationError)?
        ...
        for (tenant_id, config) in &tenant_configs {
            config.validate().map_err(|e| {
                ConfigError::ValidationError(format!(
                    "Invalid configuration for tenant {}: {}",
                    tenant_id, e
                ))
            })?;
        }
    ...
    pub fn get_cache_config(&self, tenant_id: &str, default: &CacheConfig) -> CacheConfig {
        self.get_tenant(tenant_id)
            .and_then(|t| t.cache)
            .unwrap_or_else(|| default.clone())
    }
...
mod tests {
    ...
    fn test_tenant_config_manager_list() {
        ...
        manager
            .register_tenant(TenantConfig::new("tenant_a".to_string()))
            .unwrap();
        manager
            .register_tenant(TenantConfig::new("tenant_b".to_string()))
            .unwrap();
        manager
            .register_tenant(TenantConfig::new("tenant_c".to_string()))
            .unwrap();
    ...
    fn test_tenant_config_manager_interaction_weight() {
        ...
        assert_eq!(
            manager.get_interaction_weight("tenant_a", "view"),
            Some(2.0)
        );
