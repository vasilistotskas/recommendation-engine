...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<AppConfig> {
        ...
        let defaults_json =
            serde_json::to_string(&defaults).map_err(ConfigError::SerializationError)?;
        ...
        let config = builder.build().map_err(ConfigError::FileError)?;

        // Deserialize into AppConfig
        let app_config: AppConfig = config.try_deserialize().map_err(ConfigError::FileError)?;
    ...
    fn add_legacy_env_vars(
        ...
        if let Ok(val) = std::env::var("DATABASE_MAX_CONNECTIONS") {
            builder = builder
                .set_override("database.max_connections", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("DATABASE_MIN_CONNECTIONS") {
            builder = builder
                .set_override("database.min_connections", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("DATABASE_ACQUIRE_TIMEOUT_SECS") {
            builder = builder
                .set_override("database.acquire_timeout_secs", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("DATABASE_IDLE_TIMEOUT_SECS") {
            builder = builder
                .set_override("database.idle_timeout_secs", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("DATABASE_MAX_LIFETIME_SECS") {
            builder = builder
                .set_override("database.max_lifetime_secs", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("REDIS_CONNECTION_TIMEOUT_SECS") {
            builder = builder
                .set_override("redis.connection_timeout_secs", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("COLLABORATIVE_WEIGHT") {
            builder = builder
                .set_override("algorithms.collaborative_weight", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("CONTENT_BASED_WEIGHT") {
            builder = builder
                .set_override("algorithms.content_based_weight", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("SIMILARITY_THRESHOLD") {
            builder = builder
                .set_override("algorithms.similarity_threshold", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("DEFAULT_RECOMMENDATION_COUNT") {
            builder = builder
                .set_override("algorithms.default_recommendation_count", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("MAX_RECOMMENDATION_COUNT") {
            builder = builder
                .set_override("algorithms.max_recommendation_count", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("INCREMENTAL_UPDATE_INTERVAL_SECS") {
            builder = builder
                .set_override("model_updates.incremental_update_interval_secs", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("FULL_REBUILD_INTERVAL_HOURS") {
            builder = builder
                .set_override("model_updates.full_rebuild_interval_hours", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("TRENDING_UPDATE_INTERVAL_HOURS") {
            builder = builder
                .set_override("model_updates.trending_update_interval_hours", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("MODEL_UPDATE_BATCH_SIZE") {
            builder = builder
                .set_override("model_updates.batch_size", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("RECOMMENDATION_CACHE_TTL_SECS") {
            builder = builder
                .set_override("cache.recommendation_ttl_secs", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("TRENDING_CACHE_TTL_SECS") {
            builder = builder
                .set_override("cache.trending_ttl_secs", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("USER_PREFERENCE_CACHE_TTL_SECS") {
            builder = builder
                .set_override("cache.user_preference_ttl_secs", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("ENTITY_FEATURE_CACHE_TTL_SECS") {
            builder = builder
                .set_override("cache.entity_feature_ttl_secs", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("REQUIRE_API_KEY") {
            builder = builder
                .set_override("authentication.require_api_key", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("RATE_LIMIT_REQUESTS_PER_MINUTE") {
            builder = builder
                .set_override("rate_limiting.requests_per_minute", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("RATE_LIMIT_BURST_SIZE") {
            builder = builder
                .set_override("rate_limiting.burst_size", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("METRICS_ENABLED") {
            builder = builder
                .set_override("observability.metrics_enabled", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("METRICS_PORT") {
            builder = builder
                .set_override("observability.metrics_port", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("TRACING_ENABLED") {
            builder = builder
                .set_override("observability.tracing_enabled", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("TRACING_ENDPOINT") {
            builder = builder
                .set_override("observability.tracing_endpoint", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("COLD_START_MIN_INTERACTIONS") {
            builder = builder
                .set_override("cold_start.min_interactions", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("TRENDING_WINDOW_DAYS") {
            builder = builder
                .set_override("cold_start.trending_window_days", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("POPULAR_ENTITIES_COUNT") {
            builder = builder
                .set_override("cold_start.popular_entities_count", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("DEFAULT_TENANT_ID") {
            builder = builder
                .set_override("multi_tenancy.default_tenant_id", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("WEBHOOK_RETRY_MAX_ATTEMPTS") {
            builder = builder
                .set_override("webhooks.retry_max_attempts", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("WEBHOOK_RETRY_BACKOFF_SECS") {
            builder = builder
                .set_override("webhooks.retry_backoff_secs", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("BULK_IMPORT_BATCH_SIZE") {
            builder = builder
                .set_override("bulk_operations.import_batch_size", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("BULK_IMPORT_MAX_RECORDS") {
            builder = builder
                .set_override("bulk_operations.import_max_records", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("BULK_EXPORT_BATCH_SIZE") {
            builder = builder
                .set_override("bulk_operations.export_batch_size", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("WORKER_THREADS") {
            builder = builder
                .set_override("performance.worker_threads", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("MAX_BLOCKING_THREADS") {
            builder = builder
                .set_override("performance.max_blocking_threads", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("ENABLE_CONNECTION_POOLING") {
            builder = builder
                .set_override("performance.enable_connection_pooling", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("CORS_ALLOWED_ORIGINS") {
            builder = builder
                .set_override("security.cors_allowed_origins", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("ENABLE_COMPRESSION") {
            builder = builder
                .set_override("security.enable_compression", val)
                .unwrap();
        }
        if let Ok(val) = std::env::var("MAX_REQUEST_SIZE_MB") {
            builder = builder
                .set_override("security.max_request_size_mb", val)
                .unwrap();
        }
        ...
        if let Ok(val) = std::env::var("DRAIN_REQUESTS_ON_SHUTDOWN") {
            builder = builder
                .set_override("shutdown.drain_requests", val)
                .unwrap();
        }
